...
class mem_page(File):
	...
	def parse(self):
		...
		try:
			file_number = (re.match("^.*/\D+/\d+/(\d+)\.perf", self.name)).group(1)
		except:
...
def plotdata(position, title):
	...
	pl.subplot(pl_rows, pl_cols, position + pl_cols)
...
def do_set(partition_type, algorithm, partition_utilization, position):
	...
	for item in range(1, 100):
		file_path = parent_dir + partition_type + '/' + algorithm + '/' + str(partition_utilization) + '/' + str(item) + '.perf'
		mem_page(file_path)
...
def main():
	...
	algorithms = ['buddy', 'random', 'rr', 'bb']	# Page placement algorithm used inside memory allocator
	...
	figname = 'XE_BW_' + partition_type.upper() + '_' + str(partition_utilization) + '.png'
	...
	for algorithm in algorithms:
		...
		do_set(partition_type, algorithm, partition_utilization, position)
